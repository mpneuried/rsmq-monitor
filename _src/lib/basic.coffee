...
module.exports = class Basic extends require('events').EventEmitter
	...
	redisConnect: =>
		process.nextTick =>
			...
			if @rsmqConf?.constructor?.name is "RedisClient"
				@redis = @rsmqConf
			else if @rsmqConf?.client?.constructor?.name is "RedisClient"
				@redis = @rsmqConf.client
			else
				@redis = RedisInst.createClient( @rsmqConf?.port or 6379, @rsmqConf?.host or "127.0.0.1", @rsmqConf?.options or {} )

			# write the redis Client back to the config
			@rsmqConf.client = @redis

			@connect = @redis.connected or false
			if @connect
				@emit( "connect" )
